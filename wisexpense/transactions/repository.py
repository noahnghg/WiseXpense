...
def upsert_from_plaid(db: Session, plaid_transactions: list[dict]) -> int:
...
def remove_by_plaid_ids(db: Session, plaid_ids: list[str]) -> int:
    ...
    count = (
        db.query(Transaction)
        .filter(Transaction.plaid_transaction_id.in_(plaid_ids))
